import type { ItemBaseQueryType, ItemDetailQueryType, traderForType } from "../types/Items/queryType";
...
export function itemBaseAdapter(data: ItemBaseQueryType[]): ItemBaseResultType[] {
  ...
  return data.map((item: ItemBaseQueryType)=>{
    const top = bestSeller(item.sellFor);
      ...
      bestBuy: item.buyFor.sort((a,b)=> (a.priceRUB - b.priceRUB)).map((buy)=> ({
...
export function itemDetailsAdapter(data: ItemDetailQueryType[]): ItemDetailResultType[] {
  ...
  return data.map((details: ItemDetailQueryType)=>{
    return{
      id: details.id ?? null,
      name: details.name ?? "",
      normalizedName: details.normalizedName ?? "",
      wiki: details.wikiLink ?? "",
      sellTo: details.sellFor.sort((a, b)=>(b.priceRUB - a.priceRUB)).map((sell)=>({
      ...
      buyFrom: details.buyFor.sort((a, b)=>(a.priceRUB - b.priceRUB)).map((buy)=>{
          const traderLevelIdx = buy.vendor.trader?.levels.findIndex(level => level.level === buy.vendor.minTraderLevel )
          return {
            id: "",
          ...
          playertoTraderRequirements: {
            ...
            traderIcon:"",
            traderLevel: buy.vendor.trader?.levels[traderLevelIdx].level ?? null,
            playerLevel:buy.vendor.trader?.levels[traderLevelIdx].requiredPlayerLevel ?? null,
            reputation:buy.vendor.trader?.levels[traderLevelIdx].requiredReputation ?? null,
            commerce:buy.vendor.trader?.levels[traderLevelIdx].requiredCommerce ?? null,
          },
          questRequirement: {
      ...
      barterInput: details.bartersFor.map((input)=>{
        const traderLevelIdx = input?.trader.levels.findIndex(level=> level.level === input.level);
        return{
        id: input.id ?? "",
        ...
        playertoTraderRequirements: {
          ...
          playerLevel:input.trader.levels[traderLevelIdx].requiredPlayerLevel ?? null,
          reputation:input.trader.levels[traderLevelIdx].requiredReputation ?? null,
          commerce:input.trader.levels[traderLevelIdx].requiredCommerce ?? null,
        },
        questRequirement: {
      ...
      barterOutput: details.bartersUsing.map((output)=>{
        const traderLevelIdx = output?.trader.levels.findIndex(level=> level.level === output.level);
        return{
        id: output.id ?? "",
        ...
        playertoTraderRequirements: {
          ...
          playerLevel:output.trader.levels[traderLevelIdx].requiredPlayerLevel ?? null,
          reputation:output.trader.levels[traderLevelIdx].requiredReputation ?? null,
          commerce:output.trader.levels[traderLevelIdx].requiredCommerce ?? null,
        },
        questRequirement: {
      ...
      craftInput: details.craftsFor.map(input=>{
        return{
          id: input.id ?? "",
      ...
      craftOutput: details.craftsUsing.map(output=>{
      return{
        id: output.id ?? "",
      ...
      taskNeed: details.usedInTasks.map((need)=>{
        const tasks = need.objectives.filter((task)=> task.item.name.includes(details.name))
        return{
          name: need.name,
          task: tasks.map((task)=> ({
    ...
    taskGive: details.receivedFromTasks.map((get)=>{
      return{
        name: get.name,
